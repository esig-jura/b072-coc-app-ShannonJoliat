...
function formerTroupe(coutFormation) {
    if (totalOr.value >= coutFormation) {
        totalOr.value -= coutFormation
    }
    else {
...
<template>
        <PageTopBarre :or="totalOr"></PageTopBarre>
        <PageHeader :titre="titre" :description="description" :site="site"></PageHeader>

        <main>
            <ul class="cartes">
                <li v-for="troupe in troupes" :key="troupe.id">
                  <troupe-carte :troupe="troupe" :or="totalOr" @former="formerTroupe"></troupe-carte>
                </li>
            </ul>
        </main>
